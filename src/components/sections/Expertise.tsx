...
/**
 * Capabilities — clean 3-col × 2-row grid. Server Component.
 *
 * Each card is a Next.js Link wrapping the whole article so the whole
 * card is clickable AND the video underneath still receives
 * onMouseEnter/Leave (an absolute overlay used to block those events).
 *
 * Was client-only because of Framer Motion entrance animations. The
 * embedded `<AssetVideo>` stays client (it has hover handlers + reads
 * from AssetProvider), but the section shell itself is now an RSC —
 * the staggered card entrance moved to pure CSS via `.fade-up-stagger`.
 */
          ...
          <div className="md:col-span-7">
            ...
            <h2 className="mt-6 max-w-[16ch] text-balance font-display text-display-lg leading-[0.95] tracking-tightest text-bone">
              ...
              <span className="text-mute">
                One operating system for the modern web.
              </span>
            </h2>
          </div>
          <p className="md:col-span-5 text-pretty text-mute md:text-lg">
            We don&apos;t sell deliverables — we operate the marketing engine
            that powers your category. Click any card to dive deeper.
          </p>
        </div>

        <div className="fade-up-stagger mt-16 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {EXPERTISE.map((tile) => (
            <Link
              key={tile.id}
              href="/#services"
